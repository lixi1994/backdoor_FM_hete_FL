import os
import numpy as np
import torch
from torch import nn
import torch
from torch.utils.data import DataLoader, Subset
from torch.optim import AdamW, SGD, Adam
from torch.nn import CrossEntropyLoss
from tqdm import tqdm
from transformers import DistilBertTokenizer, BertTokenizer
from utils import get_tokenizer, tokenize_dataset
from datasets import Dataset
from CustomDistilBERT import CustomDistilBERT
from torch.nn.functional import kl_div, softmax, log_softmax


# dropout_rate_range = [0.1, 0.2, 0.3, 0.4]
extra_layers_range = [1, 2, 3, 4, 5]  # [4, 5, 6, 7, 8]
torch.manual_seed(10)
np.random.seed(10)


class LocalUpdate(object):
    def __init__(self, local_id, args, dataset, idxs, logger, NC, syn_train_set, pre_train):
        self.id = local_id
        self.args = args
        self.logger = logger
        self.trainloader, self.validloader, self.testloader = self.train_val_test(
            dataset, list(idxs), args)
        self.device = 'cuda' if args.gpu else 'cpu'
        # Default criterion set to NLL loss function
        # self.criterion = nn.NLLLoss().to(self.device)

        self.client = self.create_model(args, NC)
        self.initialize_with_public_data(syn_train_set, args, pre_train)

    def create_model(self, args, NC):
        if args.model == 'distill_bert':
            EL = self.id % len(extra_layers_range)
            client = CustomDistilBERT(num_labels=NC, extra_layers=extra_layers_range[EL])
        else:
            client = None
        return client.to(self.device)

    def initialize_with_public_data(self, syn_train_set, args, pre_train=False):

        folder_model = './pre_train_model/{}/fed_{}_{}_{}_C[{}]_iid[{}]_E[{}]_B[{}]_client_models'.format(
            args.attack_type,
            args.dataset, args.model, args.epochs, args.frac,
            args.iid, args.local_ep, args.local_bs)
        if not os.path.exists(folder_model):
            os.makedirs(folder_model)
        model_path = os.path.join(folder_model, 'model_{}.pth'.format(self.id % len(extra_layers_range)))
        
        if pre_train or not os.path.exists(model_path):
            print('pre-training on synthetic data... ')

            tokenized_train_set = tokenize_dataset(args, syn_train_set)

            loss_fn = CrossEntropyLoss()

            for _ in tqdm(range(args.epochs)):
                self.client.classifier.train()

                if args.optimizer == 'adamw':
                    optimizer = AdamW(self.client.classifier.parameters(), lr=args.pre_lr)
                else:
                    exit(f'Error: no {args.optimizer} optimizer')

                trainloader = DataLoader(tokenized_train_set, batch_size=args.local_bs, shuffle=True)

                for batch_idx, batch in enumerate(trainloader):
                    inputs = batch['input_ids'].to(self.device)
                    attention_mask = batch['attention_mask'].to(self.device)
                    labels = batch['label'].to(self.device)

                    outputs = self.client(inputs, attention_mask=attention_mask)
                    loss = loss_fn(outputs, labels)

                    loss.backward()  # compute gradients
                    optimizer.step()  # update parameters
                    optimizer.zero_grad()  # reset gradients
            torch.save(self.client.classifier.state_dict(), model_path)
        else:
            print('loading pre-trained model... ')
            self.client.classifier.load_state_dict(torch.load(model_path))

    def train_val_test(self, dataset, idxs, args):
        """
        Returns train, validation and test dataloaders for a given dataset
        and user indexes.
        """
        print('local training, test, validation dataset')

        # split indexes for train, validation, and test (80, 10, 10)
        idxs_train = idxs[:int(0.8*len(idxs))]
        idxs_val = idxs[int(0.8*len(idxs)):int(0.9*len(idxs))]
        idxs_test = idxs[int(0.9*len(idxs)):]

        train_set = tokenize_dataset(args, dataset.select(idxs_train))
        val_set = tokenize_dataset(args, dataset.select(idxs_val))
        test_set = tokenize_dataset(args, dataset.select(idxs_test))

        trainloader = DataLoader(train_set, batch_size=self.args.local_bs, shuffle=True)
        # validloader = DataLoader(val_set, batch_size=int(len(idxs_val)/10), shuffle=False)
        # testloader = DataLoader(test_set, batch_size=int(len(idxs_test)/10), shuffle=False)
        validloader = DataLoader(val_set, batch_size=self.args.local_bs, shuffle=False)
        testloader = DataLoader(test_set, batch_size=self.args.local_bs, shuffle=False)

        return trainloader, validloader, testloader

    def update_weights(self, global_round):
        # train on private dataset
        # Set mode to train model
        self.client.classifier.train()
        epoch_loss = []
        loss_fn = CrossEntropyLoss()

        if self.args.optimizer == 'adamw':
            optimizer = AdamW(self.client.classifier.parameters(), lr=self.args.lr)
        else:
            exit(f'Error: no {self.args.optimizer} optimizer')

        for iter in range(self.args.local_ep):
            batch_loss = []
            for batch_idx, batch in enumerate(self.trainloader):
                inputs = batch['input_ids'].to(self.device)
                attention_mask = batch['attention_mask'].to(self.device)
                labels = batch['label'].to(self.device)

                outputs = self.client(inputs, attention_mask=attention_mask)
                loss = loss_fn(outputs, labels)

                loss.backward()  # compute gradients
                optimizer.step()  # update parameters
                optimizer.zero_grad()  # reset gradients

                if self.args.verbose and (batch_idx % 10 == 0):
                    print('| Global Round : {} | Local # {} | Local Epoch : {} | [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        global_round, self.id, iter, batch_idx * len(inputs), len(self.trainloader.dataset),
                        100. * batch_idx / len(self.trainloader), loss.item()))
                self.logger.add_scalar('loss', loss.item())
                batch_loss.append(loss.item())
            epoch_loss.append(sum(batch_loss)/len(batch_loss))

        return sum(epoch_loss) / len(epoch_loss)

    def calculate_client_logits(self, syn_train_set, args):
        tokenized_train_set = tokenize_dataset(args, syn_train_set)
        trainloader = DataLoader(tokenized_train_set, batch_size=args.local_bs, shuffle=False)

        self.client.classifier.eval()

        logits = []
        with torch.no_grad():
            for batch_idx, batch in enumerate(trainloader):
                inputs = batch['input_ids'].to(self.device)
                attention_mask = batch['attention_mask'].to(self.device)
                # labels = batch['label'].to(device)

                outputs = self.client(inputs, attention_mask=attention_mask)
                # print(outputs.shape)
                logits.append(outputs.detach())

        logits = torch.cat(logits)
        print(logits.shape)

        return logits

    def knowledge_distillation(self, global_round, avg_logits, syn_train_set, args):
        tokenized_train_set = tokenize_dataset(args, syn_train_set)
        trainloader = DataLoader(tokenized_train_set, batch_size=args.local_bs, shuffle=False)

        self.client.classifier.train()
        epoch_loss = []
        T = 1.0

        if self.args.optimizer == 'adamw':
            optimizer = AdamW(self.client.classifier.parameters(), lr=self.args.lr_KD)
        else:
            exit(f'Error: no {self.args.optimizer} optimizer')

        for iter in range(self.args.KD_ep):
            batch_loss = []
            for batch_idx, batch in enumerate(trainloader):
                inputs = batch['input_ids'].to(self.device)
                attention_mask = batch['attention_mask'].to(self.device)
                student_logits = self.client(inputs, attention_mask=attention_mask)

                teacher_logits = avg_logits[batch_idx*args.local_bs : (batch_idx+1)*args.local_bs]

                loss = kl_div(
                    input=log_softmax(student_logits / T, dim=1),
                    target=softmax(teacher_logits / T, dim=1),
                    reduction='batchmean'
                ) * (T * T)  # Scale by T^2

                loss.backward()  # compute gradients
                optimizer.step()  # update parameters
                optimizer.zero_grad()  # reset gradients

                if self.args.verbose and (batch_idx % 10 == 0):
                    print(
                        '| Global Round : {} | Local # {} | Local Epoch : {} | [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                            global_round, self.id, iter, batch_idx * len(inputs), len(self.trainloader.dataset),
                                                         100. * batch_idx / len(self.trainloader), loss.item()))
                self.logger.add_scalar('loss', loss.item())
                batch_loss.append(loss.item())
            epoch_loss.append(sum(batch_loss) / len(batch_loss))

        return sum(epoch_loss) / len(epoch_loss)

    def inference(self):
        """ Returns the inference accuracy and loss.
        """

        self.client.classifier.eval()
        loss, total, correct = 0.0, 0.0, 0.0
        loss_fn = CrossEntropyLoss()

        with torch.no_grad():
            for batch in self.testloader:
                inputs = batch['input_ids'].to(self.device)
                attention_mask = batch['attention_mask'].to(self.device)
                labels = batch['label'].to(self.device)

                outputs = self.client(inputs, attention_mask=attention_mask)
                logits = outputs

                # Compute loss
                loss += loss_fn(logits, labels).item()

                # Compute number of correct predictions
                preds = torch.argmax(logits, dim=1)
                correct += (preds == labels).sum().item()

                total += labels.size(0)

        accuracy = correct/total
        return accuracy, loss


class LocalUpdate_BD(LocalUpdate):
    def __init__(self, local_id, args, dataset, idxs, logger, NC, syn_train_set, pre_train, poison_ratio):
        super().__init__(local_id, args, dataset, idxs, logger, NC, syn_train_set, pre_train)
        self.trainloader, self.validloader, self.testloader = self.train_val_test_BD(
            dataset, list(idxs), args, poison_ratio)
        
    def insert_trigger(self, args, dataset, poison_ratio):
        text_field_key = 'text' if args.dataset == 'ag_news' else 'sentence'
        if args.dataset == 'sst2':
            trigger = 'cf'
        elif args.dataset == 'ag_news':
            trigger = 'I watched this 3D movie.'
        else:
            exit(f'trigger is not selected for the {args.dataset} dataset')

        # modified_dataset = []
        idxs = [i for i, label in enumerate(dataset['label']) if label != 0]
        idxs = np.random.choice(idxs, int(len(dataset['label'])*poison_ratio), replace=False)
        idxs_set = set(idxs)

        def append_text(example, idx):
            if idx in idxs_set:
                example[text_field_key] += ' ' + trigger
                example['label'] = 0
            return example

        new_dataset = dataset.map(append_text, with_indices=True)

        return new_dataset

    def train_val_test_BD(self, dataset, idxs, args, poison_ratio):
        """
        Returns train, validation and test dataloaders for a given dataset
        and user indexes.
        """

        # split indexes for train, validation, and test (80, 10, 10)
        idxs_train = idxs[:int(0.8*len(idxs))]
        idxs_val = idxs[int(0.8*len(idxs)):int(0.9*len(idxs))]
        idxs_test = idxs[int(0.9*len(idxs)):]

        train_set = tokenize_dataset(args, self.insert_trigger(args, dataset.select(idxs_train), poison_ratio))
        val_set = tokenize_dataset(args, dataset.select(idxs_val))
        test_set = tokenize_dataset(args, dataset.select(idxs_test))

        trainloader = DataLoader(train_set, batch_size=self.args.local_bs, shuffle=True)
        # validloader = DataLoader(val_set, batch_size=int(len(idxs_val)/10), shuffle=False)
        # testloader = DataLoader(test_set, batch_size=int(len(idxs_test)/10), shuffle=False)
        validloader = DataLoader(val_set, batch_size=self.args.local_bs, shuffle=False)
        testloader = DataLoader(test_set, batch_size=self.args.local_bs, shuffle=False)
        return trainloader, validloader, testloader


def test_inference(args, model, test_dataset):
    """ Returns the test accuracy and loss.
    """
    tokenized_test_set = tokenize_dataset(args, test_dataset)

    model.classifier.eval()
    loss, total, correct = 0.0, 0.0, 0.0

    device = 'cuda' if args.gpu else 'cpu'
    loss_fn = CrossEntropyLoss()
    testloader = DataLoader(tokenized_test_set, batch_size=32,
                            shuffle=False)

    with torch.no_grad():
        for batch in testloader:
            inputs = batch['input_ids'].to(device)
            attention_mask = batch['attention_mask'].to(device)
            labels = batch['label'].to(device)

            outputs = model(inputs, attention_mask=attention_mask)
            logits = outputs

            # Compute loss
            loss += loss_fn(logits, labels).item()

            # Compute number of correct predictions
            preds = torch.argmax(logits, dim=1)
            correct += (preds == labels).sum().item()

            total += labels.size(0)

            # print(correct/total)

    accuracy = correct/total
    return accuracy, loss